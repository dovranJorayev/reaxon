...
/**
 * @description Implementation of the resource function
 * DISCLAIMER: All tests cases are copied from the mobx-utils library, but adapted to internal use cases.
 * https://github.com/mobxjs/mobx-utils/blob/master/from-resource.ts
 */

type IDisposer = () => void;
const NOOP = () => {};
function invariant(cond: boolean, message = 'Illegal state') {
...
/**
 * `resource` creates an observable whose current state can be inspected using `.current()`,
 * and which can be kept in sync with some external datasource that can be subscribed to.
 *
 * The created observable will only subscribe to the datasource if it is in use somewhere,
 * (un)subscribing when needed. To enable `resource` to do that two callbacks need to be provided,
...
export function resource<T>(
  ...
  return {
    current: () => {
      ...
      if (!isBeingTracked && !isActive && !suppressWarning)
        console.warn(NO_SUBSCRIPTION_WARNING);
      return value;
    },
    dispose: () => {
      isDisposed = true;
